...
int main() {
  ...
  auto& dummy_counter = BuildCounter()
                             .Name("dummy_total")
                             .Help("Number of dummy")
                             .Register(*registry);

  // add and remember dimensional data, incrementing those is very cheap
  auto& tcp_rx_counter =
      dummy_counter.Add({{"protocol", "tcp"}, {"direction", "rx"}});
  auto& tcp_tx_counter =
      dummy_counter.Add({{"protocol", "tcp"}, {"direction", "tx"}});
  auto& udp_rx_counter =
      dummy_counter.Add({{"protocol", "udp"}, {"direction", "rx"}});
  auto& udp_tx_counter =
      dummy_counter.Add({{"protocol", "udp"}, {"direction", "tx"}});
